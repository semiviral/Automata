...
namespace Automata.Engine.Tests.Numerics.Vector4_Types
{
    public class Byte
    {
        private static readonly Vector4<byte> _A = new Vector4<byte>(0, 10, 10, byte.MaxValue);
        private static readonly Vector4<byte> _B = new Vector4<byte>(0, 0, 20, byte.MaxValue);

        [Fact]
        public void AddOperator()
        {
            Vector4<byte> result = _A + _B;

            Debug.Assert(result.X is 0);
            Debug.Assert(result.Y is 10);
            Debug.Assert(result.Z is 30);
            Debug.Assert(result.W is 254);
        }
        ...
        {
            Vector4<byte> result = Vector4<byte>.Abs(new Vector4<byte>(1));
