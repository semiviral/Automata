...
namespace Automata.Game.Chunks.Generation
{
    ...
    {
        private static readonly Vector3[][] _VertexesByIterationVector3 =
        {
            new[]
            {
                // East
        ...
        private static readonly Vector3i[][] _VertexesByIterationVector3i =
        {
            new[]
            {
                // East
        ...
        public static PendingMesh<int> GeneratePackedMesh(INodeCollection<ushort> blocksCompressed, INodeCollection<ushort>?[] neighbors, bool traversalMeshing)
        {
            ...
            Vector3 localPosition, ushort currentBlockId, bool isCurrentBlockTransparent)
        {
            ...
            for (int normalIndex = 0; normalIndex < 6; normalIndex++)
            {
                ...
                Span<Vector3> compressedVertexes = _VertexesByIterationVector3[normalIndex];
                vertexes.Add(localPosition + compressedVertexes[0]);
                vertexes.Add(localPosition + compressedVertexes[1]);
                vertexes.Add(localPosition + compressedVertexes[2]);
                vertexes.Add(localPosition + compressedVertexes[3]);
        ...
        private static void PackedTraverseIndex(Span<ushort> blocks, Span<Direction> faces, ICollection<int> vertexes, ICollection<uint> indexes,
            IReadOnlyList<INodeCollection<ushort>?> neighbors, int index, Vector3i localPosition, ushort currentBlockId, bool isCurrentBlockTransparent)
        {
            // iterate once over all 6 faces of given cubic space
            for (int normalIndex = 0; normalIndex < 6; normalIndex++)
            {
                ...
                // normalIndex constrained to represent the 3 axes
                int normalIndexMod3 = normalIndex % 3;

                // axis value of the current face check direction
                // example: for iteration normalIndex == 0—which is positive X—it'd be equal to localPosition.x
                int faceCheckAxisValue = localPosition[normalIndexMod3];
                ...
                {
                    // the index of the int3 traversalNormal to traverse on
                    int traversalNormalIndex = (normalIndexMod3 + perpendicularNormalIndex) % 3;
                    ...
                    // local start axis position + traversals
                    int totalTraversalLength = localPosition[traversalNormalIndex] + traversals;
                        ...
                        traversalIndex += traversalIndexStep) // increment traversal index by index step to adjust local working position
                    {
                        ...
                        if (isFaceCheckOutOfBounds)
                        {
                            int traversalAxisValue = localPosition[traversalNormalIndex] + traversals;
                            Vector3i traversalLocalPosition = localPosition.WithModifiedComponent(traversalNormalIndex, traversalAxisValue);
                            Vector3i neighborLocalPosition = Vector3i.Mod(traversalLocalPosition, GenerationConstants.CHUNK_SIZE);

                            // index into neighbor blocks collections, call .GetPoint() with adjusted local position
                            // remark: if there's no neighbor at the index given, then no chunk exists there (for instance,
                            //     chunks at the edge of render distance). In this case, return NullID so no face is rendered on edges.
                            ushort facedBlockId = neighbors[normalIndex]?.GetPoint(neighborLocalPosition) ?? BlockRegistry.NullID;
                    ...
                    Span<Vector3i> compressedVertices = _VertexesByIterationVector3i[normalIndex];

                    vertexes.Add(CompressVertex(localPosition
                                                + compressedVertices[0].WithModifiedComponent(traversalNormalIndex,
                                                    compressedVertices[traversalNormalIndex] * traversals)));

                    //_MeshData.AddVertex(compressedUv & (int.MaxValue << (GenerationConstants.CHUNK_SIZE_BIT_SHIFT * 2)));

                    vertexes.Add(CompressVertex(localPosition
                                                + compressedVertices[1].WithModifiedComponent(traversalNormalIndex,
                                                    compressedVertices[traversalNormalIndex] * traversals)));

                    //_MeshData.AddVertex(compressedUv & (int.MaxValue << GenerationConstants.CHUNK_SIZE_BIT_SHIFT));

                    vertexes.Add(CompressVertex(localPosition
                                                + compressedVertices[2].WithModifiedComponent(traversalNormalIndex,
                                                    compressedVertices[traversalNormalIndex] * traversals)));

                    //_MeshData.AddVertex(compressedUv & ~(GenerationConstants.CHUNK_SIZE_BIT_MASK << GenerationConstants.CHUNK_SIZE_BIT_SHIFT));

                    vertexes.Add(CompressVertex(localPosition
                                                + compressedVertices[3].WithModifiedComponent(traversalNormalIndex,
                                                    compressedVertices[traversalNormalIndex] * traversals)));
