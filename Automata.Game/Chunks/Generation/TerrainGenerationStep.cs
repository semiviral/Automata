...
namespace Automata.Game.Chunks.Generation
{
    public class TerrainGenerationStep : IGenerationStep
    {
        ...
        public void Generate(Vector3<int> origin, IGenerationStep.Parameters parameters, Span<ushort> blocks)
        {
            ...
            for (int y = 0; y < GenerationConstants.CHUNK_SIZE; y++)
            {
                ...
                for (int x = 0; x < GenerationConstants.CHUNK_SIZE; x++, heightmapIndex++, index++)
                {
                    ...
                    if (y == 0)
                    {
                        heightmap[heightmapIndex] = CalculateHeight(new Vector2(global.X, global.Z), parameters.Frequency, parameters.Persistence);
                    }
                    ...
                    else if ((noiseHeight < origin.Y) || (CalculateCaveNoise(global.Convert<float>(), parameters) < parameters.CaveThreshold))
                    {
                        blocks[index] = BlockRegistry.AirID;
                    }
                    else if (global.Y == noiseHeight)
                    {
                    ...
                    else if (global.Y < (noiseHeight - 3))
                    {
        ...
        private static float CalculateCaveNoise(Vector3<float> global, IGenerationStep.Parameters parameters)
        {
            ...
            float sampleA = OpenSimplexSlim.GetSimplex(parameters.Seed ^ 2, parameters.Frequency, global.AsIntrinsic()) * heightDampener;
            float sampleB = OpenSimplexSlim.GetSimplex(parameters.Seed ^ 3, parameters.Frequency, global.AsIntrinsic()) * heightDampener;
            sampleA *= sampleA;
            sampleB *= sampleB;
